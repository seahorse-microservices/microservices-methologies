...
function executeCommand(command: string, args: string[]) {
	const currentOperatingSystemIsWindows: boolean = (os.platform() == 'win32');

  return new Promise<void>((resolve, reject) => {
		let childProcess: ChildProcess;

		if(currentOperatingSystemIsWindows) childProcess = spawn('cmd', ['/c', command, ...args], { stdio: 'inherit' });
    else childProcess = spawn(command, args, { stdio: 'inherit' });
